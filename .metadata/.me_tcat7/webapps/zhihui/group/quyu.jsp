...
<html>
<s:if test="#session.lista==null">
	<c:redirect url="../initarea"></c:redirect>
...
<body data-genuitec-lp-enabled="false" data-genuitec-file-id="wc2-173" data-genuitec-path="/zhihui2/WebRoot/group/quyu.jsp">
	<jsp:include page="../common2.jsp"/>
			<!--slide bar end-->

			<!--main content start-->
			<div id="main-content" class="main-content col-md-10"
				style="border:1px solid #f2e1e8;border-width:0 0 0 1px;padding-left:0px;background-color:#ffffff" data-genuitec-lp-enabled="false" data-genuitec-file-id="wc2-173" data-genuitec-path="/zhihui2/WebRoot/group/quyu.jsp">
				...
				<div class="container-fluid">
					<div class="page-header"
						...
						<font style="font-size:24px;padding-top:-10px;color:#2679b5;">
							导游分区情况</font> <font style="color:#8089a0;font-size:14px"> <span
					...
					<div style="margin-left:20px;margin-right:20px;">
						  <!-- 选区start -->
								  <form action="../areadiv" method="post" >
								  <s:select name="option" list="#session.lista" listKey="adid" 
										listValue="areaname" />
								  	  
									<button type="button" onclick="this.form.submit()" class="btn  btn-primary btn-xs" style="color:#ffffff;margin-left:5px;">
									 <span class="glyphicon glyphicon-ok" style="margin-right:3px;"></span>选择</button>
									<br>
									<br><span  style="margin-left:0px;height:18px;ling-height:18px;color:red;font-size:20px;">目前显示:${aname }</span>
						</form>
							<div class="row" style="margin-top:20px;">
								<div class="col-md-3" style="height:300px;" id="group1">
									<table class="table table-bordered " id="table1">
										<thead>	
											...
											<td text-align="left">未分配的组</td>
											</tr>
										</thead>
										<tbody>
											<s:iterator value="#session.weifenpei" var="wfp">
											<tr>
												<td>
													${wfp.gname }
													<a href="../mydivgroup?groupid=${wfp.ggid}&myareaid=${areaid}"><button style="float:right;background-color:#ffffff;border:0px;color:#428bca">
								...
								<div class="col-md-3" id="grounp2" style="height:300px;">
									<table class="table table-bordered table-hover" id="table2">
										<thead>	
											<tr style="background-color:#ffffff">
											<td text-align="left">已分配的组</td>
											</tr>
										</thead>
										<tbody>
										<s:iterator value="#session.mygroup" var="mg">
											<tr>
												<td>
													${mg.gname }<a href="../cancelgroup?groupid=${mg.ggid}&myareaid=${areaid}" >
													<button style="float:right;background-color:#ffffff;border:0px;color:#d9534f">
								...
								<table id="dg" title="区域情况" class="easyui-datagrid" style="height:300px"
										url="../Areashow"
										toolbar="#toolbar" pagination="true"
										rownumbers="true" fitColumns="true" singleSelect="true">
									<thead>
										<tr>
											<th field="aname" width="50">
												区域</th>
								...
								<div id="toolbar">
									<a href="#" class="easyui-linkbutton"  plain="true" onclick="newUser()">
										<span class="glyphicon glyphicon-plus" style="color:green;"></span> 新增区域</a>
								...
								<div id="dlg" class="easyui-dialog" style="width:400px;height:180px;padding:10px 20px"
									...
									<div class="ftitle">区域情况</div>
									<form id="fm" method="post" novalidate>
										<div class="fitem">
											<label>区域名称</label>
											<input name="aname" class="easyui-validatebox" required="true">
										</div>
										<div >
	...
	function removeUser(){
		...
		if (row){
			$.messager.confirm('删除','确认要删除吗?',function(r){
				if (r){
					$.post('../delarea',{areaid:row.aid},function(result){
						if (result.success){
							$('#dg').datagrid('reload');	// reload the user data
						} else {
