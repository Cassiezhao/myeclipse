...
//        super(context, attrs);
        ...
        this(context, attrs, R.attr.ItemViewAttr);
    }
    ...
    public ItemView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.ItemView,
                defStyleAttr, R.style.ItemViewStyle);
        // 左右显示图片控件
        ImageView lImage = null;
        ImageView rImage = null;

        // 将属性文件的值赋给变量
        leftTextContent = a.getString(R.styleable.ItemView_left_text);
        rightTextContent = a.getString(R.styleable.ItemView_right_text);
        leftTextSize = a.getInt(R.styleable.ItemView_left_text_size, 16);
        rightTextSize = a.getInt(R.styleable.ItemView_right_text_size, 14);
        isDividerVisibility = a.getBoolean(R.styleable.ItemView_is_divider_visibility, false);
        int leftColor = a.getColor(R.styleable.ItemView_left_text_color, 0xff000000);
        int rightColor = a.getColor(R.styleable.ItemView_right_text_color, 0xffeeeeee);

        Drawable l = a.getDrawable(R.styleable.ItemView_left_img_src);
        Drawable r = a.getDrawable(R.styleable.ItemView_right_img_src);
        ...
        if (isDividerVisibility) {
            findViewById(R.id.v_divider_bottom).setVisibility(VISIBLE);
        }
        // 给控件属性赋值
    ...
    public void setDividerVisiable(Boolean visiable) {
        findViewById(R.id.v_divider_bottom).setVisibility(visiable ? VISIBLE : INVISIBLE);
    }
}
