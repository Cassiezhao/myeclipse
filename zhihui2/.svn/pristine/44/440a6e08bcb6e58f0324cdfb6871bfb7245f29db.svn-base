...
public class GuideDAO extends HibernateDaoSupport {
	...
	public List<Guide> findzaixian(int page,int rows){
		Query query=getSession().createQuery("from Guide g  where g.state=? order by guideid");
	...
	public List<Guide> findbygpid(int a){
		Query query=getSession().createQuery("from Guide g where g.guidegroup.ggid=?  order by guideid");
		query.setInteger(0, a);
		return query.list();
	}
